...
class GST_Tacotron:
    ...
    def Model_Generate(self):
        ...
        input_Dict["Mel_Length"] = tf.keras.layers.Input(shape=[], dtype=tf.int32)
        ...
        input_Dict["Token_Length"] = tf.keras.layers.Input(shape=[], dtype=tf.int32)
        ...
        tensor_Dict["Train", "Encoder"] = layer_Dict["Encoder"](
            input_Dict["Token"], training=True
        )

        if hp_Dict["GST"]["Use"]:
            tensor_Dict["Train", "GST"] = layer_Dict["Style_Token_Layer"](
                [input_Dict["GST_Mel"], input_Dict["Mel_Length"]]
            )
            tensor_Dict["Train", "Encoder"] = layer_Dict["GST_Phoneme_Encoder"](
                [tensor_Dict["Train", "Encoder"], tensor_Dict["Train", "GST"]]
            )
            if hp_Dict["X_Vector"]["Use"]:
                tensor_Dict["Train", "Encoder"] = layer_Dict["X_Vector_Concat"](
                    [tensor_Dict["Train", "Encoder"], tensor_Dict["Train", "X_Vector"]]
                )
        ...
        (
            ...
            [tensor_Dict["Train", "Encoder"], input_Dict["Mel"]], training=True
        )
        ...
        if hp_Dict["GST"]["Use"]:
            ...
            tensor_Dict["Inference", "Encoder"] = layer_Dict["GST_Phoneme_Encoder"](
                [tensor_Dict["Inference", "Encoder"], tensor_Dict["Inference", "GST"]]
            )
            if hp_Dict["X_Vector"]["Use"]:
        ...
        (
            ...
            [tensor_Dict["Inference", "Encoder"], input_Dict["Mel"]], training=False
        )
        tensor_Dict["Inference", "Export_Spectrogram"] = layer_Dict["Vocoder_Taco1"](
            tensor_Dict["Inference", "Export_Mel"], training=False
        )

        self.model_Dict = {}
        self.model_Dict["Train"] = tf.keras.Model(
            inputs=[input_Dict["Mel"], input_Dict["Token"], input_Dict["Spectrogram"]]
            + (
                [input_Dict["GST_Mel"], input_Dict["Mel_Length"]]
                if hp_Dict["GST"]["Use"]
                else []
        ...
        self.model_Dict["Inference"] = tf.keras.Model(
            inputs=[input_Dict["Mel"], input_Dict["Token"]]
            + (
                [input_Dict["GST_Mel"], input_Dict["Mel_Length"]]
                if hp_Dict["GST"]["Use"]
                else []
        ...
        if hp_Dict["GST"]["Use"]:
            self.model_Dict["GST"] = tf.keras.Model(
                inputs=[input_Dict["GST_Mel"], input_Dict["Mel_Length"]],
        ...
        self.checkpoint = tf.train.Checkpoint(
            optimizer=self.optimizer, model=self.model_Dict["Train"]
        )
    ...
    def Train_Step(
        ...
        with tf.GradientTape() as tape:
            pre_Mel_Logits, mel_Logits, stop_Logits, spectrogram_Logits = (
                self.model_Dict[
                    "Train"
                ](
                    inputs=[mels, tokens, spectrograms]
                    + ([mels, mel_lengths] if hp_Dict["GST"]["Use"] else []),
                    training=True,
                )
            )
            ...
            spectrogram_Loss = tf.reduce_mean(
                tf.abs(spectrograms[:, 1:] - spectrogram_Logits), axis=-1
            )
            if hp_Dict["Train"]["Use_L2_Loss"]:
                mel_Loss += tf.reduce_mean(tf.pow(mels[:, 1:] - mel_Logits, 2), axis=-1)
                spectrogram_Loss += tf.reduce_mean(
                    tf.pow(spectrograms[:, 1:] - spectrogram_Logits, 2), axis=-1
                )
            ...
            stop_Loss = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.sequence_mask(
                    lengths=tf.math.ceil(
                        mel_lengths / hp_Dict["Step_Reduction"]
                    ),  # stop > 0.5: Going, stop < 0.5: Done
                    maxlen=tf.math.ceil(
                        tf.shape(mel_Loss)[-1] / hp_Dict["Step_Reduction"]
                    ),
                    dtype=tf.as_dtype(policy.compute_dtype),
                ),
                logits=stop_Logits,
            ...
            pre_Mel_Loss = tf.reduce_mean(pre_Mel_Loss)
            mel_Loss = tf.reduce_mean(mel_Loss)
            stop_Loss = tf.reduce_mean(stop_Loss)

            loss = (
                tf.reduce_mean(pre_Mel_Loss)
                + tf.reduce_mean(mel_Loss)
                + tf.reduce_mean(stop_Loss)
                + tf.reduce_mean(spectrogram_Loss)
            )  # +gst_phoneme_l2

        gradients = tape.gradient(loss, self.model_Dict["Train"].trainable_variables)
        self.optimizer.apply_gradients(
            zip(gradients, self.model_Dict["Train"].trainable_variables)
        )

        # return loss #, gst_phoneme_l2
        return loss, pre_Mel_Loss, mel_Loss, stop_Loss
    ...
    def Inference_Step(
        ...
        initial_mels,
        mels_for_gst=None,
        mel_lengths_for_gst=None,
    ):
        mel_Logits, stop_Logits, spectrogram_Logits, alignments = self.model_Dict[
            "Inference"
        ](
            inputs=[initial_mels, tokens]
            + ([mels_for_gst, mel_lengths_for_gst] if hp_Dict["GST"]["Use"] else []),
            training=False,
        )
    ...
    def Inference_GST_Step(self, mels_for_gst, mel_lengths_for_gst):
        ...
        gst = self.model_Dict["GST"](
            inputs=[mels_for_gst, mel_lengths_for_gst], training=False
        )
    ...
    def Restore(self, checkpoint_File_Path=None):
        if checkpoint_File_Path is None:
            checkpoint_File_Path = tf.train.latest_checkpoint(
    ...
    def Train(self):
        if not os.path.exists(
            os.path.join(hp_Dict["Inference_Path"], "Hyper_Parameters.json")
        ):
            os.makedirs(hp_Dict["Inference_Path"], exist_ok=True)
            with open(
                os.path.join(
                    hp_Dict["Inference_Path"], "Hyper_Parameters.json"
                ).replace("\\", "/"),
                "w",
            ) as f:
                json.dump(hp_Dict, f, indent=4)

        log_dir = hp_Dict["Log_Path"]
        train_summary_writer = tf.summary.create_file_writer(log_dir)

        def Save_Checkpoint():
            os.makedirs(
                os.path.join(hp_Dict["Checkpoint_Path"]).replace("\\", "/"),
                exist_ok=True,
            )
        ...
        while True:
            ...
            loss, pre_Mel_Loss, mel_Loss, stop_Loss = self.Train_Step(
                **self.feeder.Get_Pattern()
            )
            if np.isnan(loss):
                raise ValueError("NaN loss")

            with train_summary_writer.as_default():
                tf.summary.scalar("loss", loss, self.optimizer.iterations.numpy())
                tf.summary.scalar(
                    "learning_rate",
            ...
            if (
                self.optimizer.iterations.numpy()
                % hp_Dict["Train"]["Checkpoint_Save_Timing"]
                == 0
            ):
                Save_Checkpoint()

            if (
                self.optimizer.iterations.numpy() % hp_Dict["Train"]["Inference_Timing"]
                == 0
            ):
                Run_Inference()
            ...
            if (
                self.optimizer.iterations.numpy()
                % (hp_Dict["Train"]["Inference_Timing"] * 10)
                == 0
            ):
                Run_GST_Inference()
    ...
    def Inference(self, sentence_List, wav_List_for_GST=None, label=None):
        ...
        pattern_Dict = self.feeder.Get_Inference_Pattern(
            sentence_List, wav_List_for_GST
        )
        if pattern_Dict is None:
            print("Inference fail.")
            return
        mels, stops, spectrograms, alignments = self.Inference_Step(**pattern_Dict)
    ...
    def Export_Inference(
        ...
        os.makedirs(
            os.path.join(hp_Dict["Inference_Path"], "Plot").replace("\\", "/"),
            exist_ok=True,
        )
        os.makedirs(
            os.path.join(hp_Dict["Inference_Path"], "Wav").replace("\\", "/"),
            exist_ok=True,
        )

        for index, (sentence, mel, stop, spect, alignment) in enumerate(
            zip(sentence_List, mel_List, stop_List, spectrogram_List, alignment_List)
        ):
            ...
            slice_Index = (
                np.argmax(stop < 0) if any(stop < 0) else stop.shape[0]
            )  # Check stop tokens
            ...
            new_Sig = inv_spectrogram(
                spectrogram=np.transpose(
                    spect[: np.maximum(1, slice_Index) * hp_Dict["Step_Reduction"]]
                ),
    ...
    def Export_GST(self, wav_List, tag_List, gst_List, label):
        os.makedirs(
            os.path.join(hp_Dict["Inference_Path"], "GST").replace("\\", "/"),
            exist_ok=True,
        )

        title_Column_List = ["Wav", "Tag"] + [
            "Unit_{}".format(x) for x in range(gst_List[0].shape[0])
        ]
        ...
        with open(
            os.path.join(
                hp_Dict["Inference_Path"], "GST", "{}.GST.TXT".format(label)
            ).replace("\\", "/"),
            "w",
        ) as f:
            f.write("\n".join(export_List))
