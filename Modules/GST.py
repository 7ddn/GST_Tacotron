...
from Modules.Layers.PPG_Layer import PPG_RNN, BDLayer
...
class Reference_Encoder(tf.keras.Model):
    def __init__(self):
        ...
        self.layer_Dict["BD"] = BDLayer(
            file_path=hp_Dict["GST"]["Reference_Encoder"]["BD"]["Count_Path"]
        )

        ppg_layer = PPG_RNN(factor=1, token_count=self.layer_Dict["BD"].num_class)
        ppg_layer.trainable = False
        cp_dir = hp_Dict["GST"]["Reference_Encoder"]["PPG_RNN"]["Checkpoint_Path"]
    ...
    def call(self, inputs):
        ...
        sim_mat = self.layer_Dict["BD"](ppgs)

        new_Tensor = self.layer_Dict["Encoder_RNN"](sim_mat)  # [Batch, RNN_Dim]

        return self.layer_Dict["Dense"](new_Tensor)


class Style_Token_Layer(
    tf.keras.layers.Layer
):  # Attention which is in layer must be able to access directly.
    def __init__(self):
    ...
    def build(self, input_shape):
        ...
        self.gst_tokens = self.add_weight(
            name="gst_tokens",
            shape=[
                hp_Dict["GST"]["Style_Token"]["Size"],
                hp_Dict["GST"]["Style_Token"]["Embedding"]["Size"],
            ],
    ...
    def call(self, inputs, if_mean=False):
        ...
        new_Tensor = self.layer_Dict["Reference_Encoder"](
            [mels_for_gst[:, 1:], mel_lengths]
        )  # Initial frame deletion
...
class GST_Phoneme_Encoder(tf.keras.layers.Layer):
    def __init__(self):
        ...
        for i in range(hp_Dict["GST"]["Style_Token"]["Attention"]["Size"]):
            self.layer_Dict[f"Dense_{i}"] = tf.keras.layers.Dense(
                ...
                kernel_regularizer=L2UnitRegularizer(
                    1e-4 if hp_Dict["GST"]["Phoneme_Layer"]["Use_L2"] else 0
                ),
                use_bias=False,
                activation=hp_Dict["GST"]["Phoneme_Layer"]["Activation"],
            )
    ...
    def call(self, inputs):
        ...
        if hp_Dict["GST"]["Phoneme_Layer"]["Softmax"]:
            gsts = tf.nn.softmax(
                gsts[:, :, tf.newaxis, tf.newaxis], axis=1
            )  # [Batch, GST_Dim, 1, 1]
        else:
            gsts = gsts[:, :, tf.newaxis, tf.newaxis]

        embedded = [
            self.layer_Dict[f"Dense_{i}"](encoders)
            for i in range(hp_Dict["GST"]["Style_Token"]["Attention"]["Size"])
        ]  # GST_Dim * [Batch, Time_Step, new_Dim]
        ...
        if not hp_Dict["GST"]["Phoneme_Layer"]["Use_L2"]:
            new_Tensor += encoders
